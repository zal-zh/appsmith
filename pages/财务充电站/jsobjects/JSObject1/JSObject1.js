export default {
	initData: () => {
		...
		MyCustomEvent.emit('MY_FIXED_TOP_FULL_WIDTH', {
			detail: { type: 'FIXED_TOP_FULL_WIDTH', class: 't--widget-header' },
			bubbles: true,
			cancelable: true
		});
	},
	...
	tabClick: async () => {
		if (Tabs1.selectedTab === '领取资料') {
			...
			MyCustomEvent.emit('MY_SCROLL_DOM_BY_CLASS', {
				detail: { type: 'SCROLL_DOM_BY_CLASS', class: 't--widget-imagecarousel' },
				bubbles: true,
				cancelable: true
			});
			...
			MyCustomEvent.emit('MY_SCROLL_DOM_BY_CLASS', {
				detail: { type: 'SCROLL_DOM_BY_CLASS', class: 't--widget-title2' },
				bubbles: true,
				cancelable: true
			});
		}
